...
class InterestsSpider(scrapy.Spider):
    name = "interests"
    ...
    def parse(self, response):
        url = response.css("#Table1 a").xpath("@href").getall()[1]
        file_data = requests.get("https://www.senat.ro/" + url)
        open("../output/declaratii_interes/" + self.group_list[response.request.url] + ".pdf", "wb").write(file_data.content)
